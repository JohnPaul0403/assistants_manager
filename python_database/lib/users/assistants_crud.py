...
def read_assistants(cursor, user_id: str, token: str) -> any:
    """
    ...
    try:
        ...
        assistants = list(map(lambda x : {"id": x[0], "name": x[1], "assistant_id": x[3], "token" : token, "gpt_model": x[4], "files": [{"id": file[0], "file_id" : file[1], "assistant_id" : file[2]} for file in read_files(cursor, x[3])]}, result))
        return assistants
