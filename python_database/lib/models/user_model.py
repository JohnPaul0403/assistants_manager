...
class User:
    ...
    def get_assistants(function):
        ...
        def wrapper(self, assistants):
            ...
            cursor = conn.cursor()
            get_assistants = assistants_crud.read_assistants(cursor, self.id, self.token)
    ...
    def get_id_from_db(self) -> int:
        """
        Gets the ID of the user from the database.
        ...
        cursor = conn.cursor()
        id = user_crud.read_user(cursor, self.email)[0]
        conn.close()
        return id
    ...
    def set_token(self, token: str):
        """
        Sets the token for the user in the database.
        Returns:
            bool: True if the token was successfully set, False otherwise.
        """
        conn = user_crud.connect_to_database()
        cursor = conn.cursor()
        user = user_crud.set_token(cursor, {"user_id": self.id, "token": token})
        conn.commit()
        conn.close()
        return user
...
def from_json(data: dict):
    ...
    user.assistants = data["assistants"]
    return user